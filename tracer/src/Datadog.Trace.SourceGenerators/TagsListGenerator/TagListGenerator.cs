...
namespace Datadog.Trace.SourceGenerators.TagsListGenerator
{
    ...
    public class TagListGenerator : IIncrementalGenerator
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var group in classes.GroupBy(x => x.SyntaxTree))
            {
                ...
                foreach (ClassDeclarationSyntax classDec in group)
                {
                    ...
                    var baseClassName = classDec.BaseList?.Types.FirstOrDefault()?.Type.ToString() ?? string.Empty;
        ...
        internal readonly struct TagList
        {
            ...
            public TagList(string nameSpace, string className, string baseClassName, List<PropertyTag>? tagProperties, List<PropertyTag>? metricProperties)
            {
                Namespace = nameSpace;
                ClassName = className;

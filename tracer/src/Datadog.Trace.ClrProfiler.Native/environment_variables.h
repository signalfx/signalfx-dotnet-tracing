...
namespace environment {
    ...
    const shared::WSTRING tracing_enabled = WStr("SIGNALFX_TRACE_ENABLED");

    // Sets whether debug mode is enabled. Default is false.
    const shared::WSTRING debug_enabled = WStr("SIGNALFX_TRACE_DEBUG");

    // Sets the path to the profiler's home directory, for example:
    // "C:\Program Files\SignalFx .NET Tracing\" or "/opt/signalfx/"
    const shared::WSTRING profiler_home_path = WStr("SIGNALFX_DOTNET_TRACER_HOME");
    ...
    const shared::WSTRING include_process_names = WStr("SIGNALFX_PROFILER_PROCESSES");

    // Sets the filename of executables the profiler cannot attach to.
    // If not defined (default), the profiler will attach to any process.
    // Supports multiple values separated with comma, for example:
    // "MyApp.exe,dotnet.exe"
    const shared::WSTRING exclude_process_names = WStr("SIGNALFX_PROFILER_EXCLUDE_PROCESSES");

    // Sets the Agent's host. Default is localhost.
    const shared::WSTRING agent_host = WStr("SIGNALFX_AGENT_HOST");

    // Sets the Agent's port. Default is 8126.
    const shared::WSTRING agent_port = WStr("SIGNALFX_TRACE_AGENT_PORT");

    // Sets the "deployment.environment" tag for every span.
    const shared::WSTRING env = WStr("SIGNALFX_ENV");
    ...
    const shared::WSTRING service_name = WStr("SIGNALFX_SERVICE_NAME");

    // Sets the "service_version" tag for every span that belong to the root service (and not an external service).
    const shared::WSTRING service_version = WStr("SIGNALFX_VERSION");
    ...
    const shared::WSTRING clr_disable_optimizations = WStr("SIGNALFX_CLR_DISABLE_OPTIMIZATIONS");

    // Indicates whether the profiler is running in the context
    // of Azure App Services
    const shared::WSTRING azure_app_services = WStr("SIGNALFX_AZURE_APP_SERVICES");

    // In AAS, Indicates whether the customer will need the trace agent even thoug
    // automatic tracing is disabled
    const shared::WSTRING aas_needs_agent = WStr("SIGNALFX_AAS_ENABLE_CUSTOM_TRACING");

    // In AAS, Indicates whether the customer will need dogstatsd even thoug
    // automatic tracing is disabled
    const shared::WSTRING aas_needs_dogstatsd = WStr("SIGNALFX_AAS_ENABLE_CUSTOM_METRICS");
    ...
    const shared::WSTRING azure_functions_enabled = WStr("SIGNALFX_TRACE_AZURE_FUNCTIONS_ENABLED");

    // Enable the profiler to dump the IL original code and modification to the log.
    const shared::WSTRING dump_il_rewrite_enabled = WStr("SIGNALFX_DUMP_ILREWRITE_ENABLED");

    // Sets whether to enable JIT inlining
    const shared::WSTRING clr_enable_inlining = WStr("SIGNALFX_CLR_ENABLE_INLINING");

    // Enables the compatibility with other versions of Datadog.Trace
    const shared::WSTRING internal_version_compatibility = WStr("SIGNALFX_INTERNAL_TRACE_VERSION_COMPATIBILITY");

    // Sets whether to enable NGEN images.
    const shared::WSTRING clr_enable_ngen = WStr("SIGNALFX_CLR_ENABLE_NGEN");

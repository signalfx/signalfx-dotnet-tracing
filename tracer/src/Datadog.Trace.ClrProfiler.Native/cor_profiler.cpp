...
HRESULT STDMETHODCALLTYPE CorProfiler::AssemblyLoadFinished(AssemblyID assembly_id, HRESULT hr_status)
{
    ...
    const auto is_instrumentation_assembly = assembly_info.name == WStr("SignalFx.Instrumentation");
...
HRESULT STDMETHODCALLTYPE CorProfiler::ModuleLoadFinished(ModuleID module_id, HRESULT hr_status)
{
    ...
    {
        Logger::Info("ModuleLoadFinished: SignalFx.Instrumentation.ClrProfiler.Managed.Loader loaded into AppDomain ", app_domain_id, " ",
...
#ifdef LINUX
extern uint8_t dll_start[] asm("_binary_SignalFx_Instrumentation_ClrProfiler_Managed_Loader_dll_start");
extern uint8_t dll_end[] asm("_binary_SignalFx_Instrumentation_ClrProfiler_Managed_Loader_dll_end");

extern uint8_t pdb_start[] asm("_binary_SignalFx_Instrumentation_ClrProfiler_Managed_Loader_pdb_start");
extern uint8_t pdb_end[] asm("_binary_SignalFx_Instrumentation_ClrProfiler_Managed_Loader_pdb_end");
#endif
                                             ...
                                             int* symbolsSize) const
{
    ...
    *pSymbolsArray = (BYTE*) pdb_start;
    ...
    for (auto i = 0; i < imgCount; i++)
    {
        ...
        if (name.rfind("SignalFx.Instrumentation.ClrProfiler.Native.dylib") != std::string::npos)
        {

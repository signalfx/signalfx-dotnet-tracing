...
HRESULT STDMETHODCALLTYPE CorProfiler::Initialize(IUnknown* cor_profiler_info_unknown)
{
    ...
    if (IsTracingDisabled())
    {
        if (IsAzureAppServices() && (NeedsAgentInAAS() || NeedsDogstatsdInAAS()))
        {
            ...
            Logger::Info("TRACER DIAGNOSTICS - In AAS, automatic tracing is disabled but keeping the profiler up to start child processes.");
        }
        else
        {
    ...
    if (FAILED(hr))
    {
        Logger::Warn("TRACER DIAGNOSTICS - Failed to attach profiler: interface ICorProfilerInfo7 not found.");
        return E_FAIL;
    }
...
{
    auto strValue = shared::GetEnvironmentValue(WStr("SIGNALFX_PROFILING_ENABLED"));
...
HRESULT CorProfiler::TryRejitModule(ModuleID module_id)
{
    ...
    if (module_info.assembly.name == managed_profiler_name)
    {
        ...
        {
            auto profiler_library_path = shared::GetEnvironmentValue(WStr("SIGNALFX_INTERNAL_PROFILING_NATIVE_ENGINE_PATH"));

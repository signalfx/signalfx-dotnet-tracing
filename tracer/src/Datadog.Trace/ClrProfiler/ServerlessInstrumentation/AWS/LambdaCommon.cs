    ...
    internal class LambdaCommon
    {
        ...
        internal static Scope CreatePlaceholderScope(Tracer tracer)
        {
            ...
            try
            {
                ...
                {
                    string traceId = response.Headers.Get(DDHttpHeaderNames.TraceId);
                    // need to set the exact same spanId so nested spans (auto-instrumentation or manual) will have the correct parent-id
                    string spanId = response.Headers.Get(DDHttpHeaderNames.SpanId);
                    Serverless.Debug($"received traceId = {traceId} and spanId = {spanId}");
                    var span = tracer.StartSpan(PlaceholderOperationName, null, serviceName: PlaceholderServiceName, traceId: TraceId.CreateFromUlong(Convert.ToUInt64(traceId)), spanId: Convert.ToUInt64(spanId));

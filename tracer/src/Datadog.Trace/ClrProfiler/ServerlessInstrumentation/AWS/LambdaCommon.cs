        ...
        internal static Scope CreatePlaceholderScope(Tracer tracer, ILambdaExtensionRequest requestBuilder)
        {
            ...
            try
            {
                ...
                using (var response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    var spanId = response.Headers.Get(DDHttpHeaderNames.SpanId);
                    Serverless.Debug($"received traceId = {traceId} and spanId = {spanId}");
                    var span = tracer.StartSpan(PlaceholderOperationName, null, serviceName: PlaceholderServiceName, traceId: TraceId.CreateFromUlong(Convert.ToUInt64(traceId)), spanId: Convert.ToUInt64(spanId), addToTraceContext: false);

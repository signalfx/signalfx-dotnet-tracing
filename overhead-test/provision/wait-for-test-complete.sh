...
while [ 1 == 1 ] ; do
  PASS=$(ssh -o "LogLevel=ERROR" -i ~/.orca/id_rsa  splunk@${TESTBOX_HOST} "cat /tmp/passnum.txt")
...
while [ 1 == 1 ] ; do
  RUNNING=$(ssh -o "LogLevel=ERROR" -i ~/.orca/id_rsa  splunk@${TESTBOX_HOST} "cat /tmp/tests-running")
  if [ "$RUNNING" == "0" ] ; then
    break
  fi
  poll_wait

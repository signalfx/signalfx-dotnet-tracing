...
then
	...
	export SIGNALFX_DOTNET_PROFILER_HOME=`dirname $(realpath $BASH_SOURCE)`
else
    echo "Using the specified command line parameter to configure the DD-Prof-DotNet Alpha Deploy Directory."
	export SIGNALFX_DOTNET_PROFILER_HOME=$1
fi


export CORECLR_ENABLE_PROFILING=1
export CORECLR_PROFILER={BD1A650D-AC5D-4896-B64F-D6FA25D6B26A}
export CORECLR_PROFILER_PATH_64=${SIGNALFX_DOTNET_PROFILER_HOME}/Datadog.AutoInstrumentation.Profiler.Native.x64.so
export CORECLR_PROFILER_PATH_32=${SIGNALFX_DOTNET_PROFILER_HOME}/Datadog.AutoInstrumentation.Profiler.Native.x86.so

export COMPlus_EnableDiagnostics=1
export SIGNALFX_PROFILING_ENABLED=1
...
export LD_LIBRARY_PATH=${SIGNALFX_DOTNET_PROFILER_HOME}
export LD_PRELOAD=${SIGNALFX_DOTNET_PROFILER_HOME}/Datadog.Linux.Wrapper.x64.so

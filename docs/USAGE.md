...
|-|-|-|
| `SIGNALFX_ACCESS_TOKEN` | The access token for your SignalFx organization. It enables sending traces directly to a SignalFx ingestion endpoint. To do so, the `SIGNALFX_TRACE_AGENT_URL` must be set with: `https://ingest.<REALM>.signalfx.com/v2/trace`. |  |
...
| `SIGNALFX_TRACE_AGENT_URL` | The URL to where send the traces for `Zipkin` and `DatadogAgent` exporters (see: `SIGNALFX_EXPORTER`). | `http://localhost:8126` | 

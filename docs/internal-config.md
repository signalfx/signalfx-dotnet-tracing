...
| `SIGNALFX_THREAD_SAMPLING_ENABLED` | Enable to activate thread sampling sent to SignalFx. | `false` |
| `SIGNALFX_THREAD_SAMPLING_PERIOD` | Sampling period to be used. The Tracer uses it to stop threads for shorto amount of time and fetch all stack traces. This value cannot be lower that `1000` ms. | `1000` |
